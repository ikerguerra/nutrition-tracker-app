...
export default function DashboardScreen() {
    ...
    const renderRecentMeals = () => {
        ...
        if (allEntries.length === 0) {
            return (
                <Text className="text-gray-500 dark:text-gray-400 text-center mt-4">No meals logged yet today.</Text>
            );
        }

        // Show up to 3 most recent entries
        return allEntries.slice(0, 3).map((entry) => (
            <View key={entry.id} className="bg-white dark:bg-zinc-900 rounded-2xl p-4 border border-gray-100 dark:border-zinc-800 mb-2">
                <View className="flex-row justify-between items-center">
                    ...
                    <Text className="font-bold text-black dark:text-white">{Math.round(entry.calories)} kcal</Text>
                </View>
            </View>
        ));
    };
                ...
                <View className="p-6 pt-4">
                    ...
                    <View className="bg-green-600 rounded-2xl p-5 mb-6 shadow-sm">
                        <Text className="text-lg font-bold text-white mb-4">
                            {t('dashboard.addFoodTitle') || 'What did you eat today?'}
                        </Text>
                        <TouchableOpacity className="flex-row items-center bg-white/20 rounded-xl p-3">
                            <Search size={20} color="#ffffff" className="mr-3" />
                            <Text className="text-green-50 font-medium">
                                {t('dashboard.searchFoodBtn') || 'Search foods...'}
                            </Text>
                        </TouchableOpacity>
                    </View>
                    ...
                    {loading ? (
                        ...
                        <View className="flex-row space-x-4 mb-8">
                            <View className="flex-1 bg-white dark:bg-zinc-900 rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-zinc-800 mr-2">
                                <Text className="text-gray-500 dark:text-gray-400 text-sm font-medium mb-1">Calories</Text>
                            ...
                            <View className="flex-1 bg-white dark:bg-zinc-900 rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-zinc-800 ml-2">
                                <Text className="text-gray-500 dark:text-gray-400 text-sm font-medium mb-1">Protein</Text>
                                <Text className="text-xl font-bold text-black dark:text-white">
                    ...
                    <View>
                        <View className="flex-row justify-between items-center mb-4">
                            <Text className="text-xl font-bold text-black dark:text-white">Recent Meals</Text>
                            <TouchableOpacity>
                                <Text className="text-green-600 font-medium">View All</Text>
                            </TouchableOpacity>
                        </View>

                        {loading ? <ActivityIndicator size="small" color="#16a34a" /> : renderRecentMeals()}

                        <TouchableOpacity className="bg-dashed border-2 border-dashed border-gray-300 dark:border-zinc-700 rounded-2xl p-4 flex-row justify-center items-center mt-4">
                            <Plus size={20} color="#6b7280" className="mr-2" />
                            <Text className="text-gray-500 dark:text-gray-400 font-medium">Log your next meal</Text>
                        </TouchableOpacity>
                    </View>
